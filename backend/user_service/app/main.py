...
def send_verification_otp(request: schemas.OTPRequest, db: Session = Depends(get_db)):
    """Send OTP for email verification during registration"""
    result = auth_utils.send_verification_otp(db, request.email)
...
def verify_email_otp(request: schemas.VerifyEmailOTP, db: Session = Depends(get_db)):
    ...
    is_valid = auth_utils.verify_otp(db, request.email, request.otp, "verification")
...
def forgot_password(request: schemas.PasswordResetRequest, db: Session = Depends(get_db)):
    ...
    result = auth_utils.send_password_reset_otp(db, request.email)
...
def verify_reset_otp(request: schemas.PasswordResetVerify, db: Session = Depends(get_db)):
    ...
    # Check if the OTP is valid
    is_valid = auth_utils.verify_otp(db, request.email, request.otp, "password_reset")
